#ifndef TOPSORT_TARJANNODE_HPP
#define TOPSORT_TARJANNODE_HPP

#include <iostream>
#include <set>
#include <string>
#include <vector>
using std::vector, std::set, std::string, std::cout;
...
template <typename T> void TarjanNode<T>::checkForCycle()
{
    if (cycleExists)
    {
        ...
        for (unsigned int i = 0; i < cycleSize; ++i)
        {
            if (cycleSize == i + 1)
            {
                cout << cycle[i]->getTarjanNodeName() <<
                           " Depends On " <<cycle[0]->getTarjanNodeName() << "\n";
            }
            else
            {
                cout << cycle[0]->getTarjanNodeName() <<
                     " Depends On " <<cycle[0]->getTarjanNodeName() << "\n";
            }
        }
        exit(EXIT_SUCCESS);
...
struct Dependency{
    set<T*> dependencies;
    set<T*> dependents;
    set<T*> allDependencies;
};

#endif // TOPSORT_TARJANNODE_HPP
